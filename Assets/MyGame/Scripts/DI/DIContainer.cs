...
namespace DI
{
    public class DIContainer
    {
        ...
        public void RegisterSinglton<T>(Func<DIContainer, T> factory)
        {
            RegisterSinglton ( null, factory);

        }
        public void RegisterSinglton<T>(string tag, Func<DIContainer, T> factory)
        {
        ...
        public void RegisterInstance<T>(string tag, T instance)
        {
            ...
            {
                throw new Exception(
                    message: $"DI: Factory with tag {key.Item1} and type {key.Item2.FullName}has already registration");
            }
        ...
        public T Resolve<T>(string tag = null)
        {
            ...
            {
                throw new Exception($"Cuclic dependency for tag{tag} and type {key.Item2.FullName}");
            }
        ...
        private void Register<T>((string, Type) key, Func<DIContainer, T> factory, bool IsSinglton)
        {
            if (_registrations.ContainsKey(key))
            {
                throw new Exception(
